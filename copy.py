...
layout = [
    [sg.Text('Wybierz plik do skonwertowania')],
    [sg.FileBrowse('Przeglądaj', file_types=(("Pdf Files", '*.pdf'),), key='input_file')],
    [sg.Text('Podaj ilość stron'), sg.InputText(key='n_pages')],
    [sg.Text('Podaj nazwę wyjściową pliku'), sg.InputText(key='output_name')],
    [sg.Text('Podaj media'), sg.InputText(key='media_output_name')],
    [sg.OK(), sg.Cancel()]
]
